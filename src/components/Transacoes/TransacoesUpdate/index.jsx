...
export const TransacoesUpdate = ({ transacaoId }) => {
    ...
    const onSubmit = async (event) => {
        ...
        try{
            const token = localStorage.getItem('token');
            const response = await axios.put(`http://localhost:8080/transacoes/${ transacaoId }`, { descricao, valor, data: dataTransacao, tipo, categoria_id: categoria, user_id: userId }, {
                headers: {
                    Authorization: `Bearer ${ token }`
                }
            })
    ...
    return (
        <>
            <S.Form onSubmit={ onSubmit }>
                ...
                <S.TextField type='text' onChange={onChangeValue} value={descricao} placeholder='Nome' name="descricao" label="Descricao" variant="outlined" fullWidth/>
                <S.TextField type='text' onChange={onChangeValue} value={valor} placeholder='Valor' name="valor" label="Valor" variant="outlined" fullWidth/>
                <S.TextField type='text' onChange={onChangeValue} value={dataTransacao} name="data" label="Data" variant="outlined" fullWidth/>
